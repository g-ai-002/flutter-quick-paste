...
- **任务**:
  - [x] 热键可配置化：main.dart 读取已存储的热键配置，SettingsPage 添加热键设置 UI
  - [x] 提取 EditPresetDialog 独立 widget
  - [x] 改进 PasteService：添加超时和退出码检查
  - [x] 改进 StorageService：确保 _init 只调用一次，防止并发初始化
  - [x] 代码清理：移除未使用 import，统一代码风格
  - [x] 更新测试用例
  - [x] 更新版本号到 0.2.2

### v0.2.1 (PATCH)
- **状态**: 开发完成 ✅
- **目标**: 修复 Issue #1 — 实现全局热键弹出/隐藏窗口
- **任务**:
  - [x] 添加 hotkey_manager 依赖
  - [x] 注册全局热键 Ctrl+Shift+V 显示/隐藏窗口
  - [x] 窗口启动时隐藏，热键弹出
  - [x] 粘贴后自动隐藏窗口
  - [x] 更新测试用例
  - [x] 更新版本号到 0.2.1
